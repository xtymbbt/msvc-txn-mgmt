...
func initDB() (err error) {
	log.Println("connecting to database in "+config.DBUrl+" at port 3306.")
	for _, dbName := range config.DBNAME {
		...
		if err = db[dbName].Ping(); err != nil {
			log.Fatalf("open database failed, error is: %v\n", err)
			return
		}
	}
	log.Println("successfully connected to database in "+config.DBUrl+" at port 3306.")
	return nil
}
